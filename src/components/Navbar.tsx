        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-45">
            ...
            <div className="hidden md:flex space-x-8 items-center">
              ...
              {user && (
                <>
                  <Link href="/favorites" className="text-gray-800 hover:text-amber-900 transition font-medium text-lg relative group">
                    Favorites
                    <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-amber-700 group-hover:w-full transition-all duration-300 rounded-full" />
                  </Link>
                  {isAdmin && (
                    <Link href="/admin" className="text-gray-800 hover:text-amber-900 transition font-medium text-lg relative group">
                      Admin
                      <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-amber-700 group-hover:w-full transition-all duration-300 rounded-full" />
                    </Link>
                  )}
              ...
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="block text-gray-800 hover:text-amber-900 hover:bg-amber-100 px-4 py-3 rounded-lg transition font-medium text-lg"
                  >
                    {link.label}
                  </Link>
                </motion.div>
              ...
              {user && (
                ...
                >
                  <div className="border-t border-amber-200 my-4 pt-4">
                    <Link href="/favorites" onClick={() => setMobileMenuOpen(false)} className="block text-gray-800 hover:text-amber-900 hover:bg-amber-100 px-4 py-3 rounded-lg transition font-medium text-lg">
                      Favorites
                    </Link>
                    {isAdmin && (
                      <Link href="/admin" onClick={() => setMobileMenuOpen(false)} className="block text-gray-800 hover:text-amber-900 hover:bg-amber-100 px-4 py-3 rounded-lg transition font-medium text-lg">
                        Admin
                      </Link>
                    )}
                  </div>
